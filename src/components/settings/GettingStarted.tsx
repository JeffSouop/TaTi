    ...
    <div className="space-y-6">
      <Card className="p-5">
        <h2 className="font-semibold mb-2">
          1. Choisir un moteur IA open source (pas seulement Ollama)
        </h2>
        <p className="text-sm text-muted-foreground mb-3">
          TaTi peut se connecter a n'importe quelle API LLM compatible OpenAI. Tu peux utiliser
          Ollama, LM Studio, vLLM, TGI, OpenRouter (modeles OSS), etc.
        </p>
        <p className="text-xs font-medium mb-1.5 mt-3">
          Option A — Ollama (le plus simple en local)
        </p>
        <CodeBlock
...
# Endpoint: http://localhost:8000/v1`}
      ...
      <Card className="p-5">
        ...
        <p className="text-sm text-muted-foreground mb-3">
          Cette app tourne dans le cloud — elle ne peut pas joindre{" "}
          <code className="text-xs bg-muted px-1 rounded">localhost</code>. Choisis ngrok (rapide)
          ou cloudflared (gratuit, pas de session limitée). Remplace le port selon ton moteur:
          Ollama <code className="text-xs bg-muted px-1 rounded">11434</code>, LM Studio{" "}
          <code className="text-xs bg-muted px-1 rounded">1234</code>, vLLM/TGI{" "}
          <code className="text-xs bg-muted px-1 rounded">8000</code>.
        </p>
...
# Copie l'URL https://xxxx.ngrok-free.app et colle-la dans l'onglet "Endpoint IA"`}
        />
        <p className="text-xs font-medium mb-1.5 mt-3">
          Option B — Cloudflare Tunnel (gratuit, illimité)
        </p>
        <CodeBlock
          code={`# Exemple Ollama
...
# Copie l'URL https://xxx.trycloudflare.com et colle-la dans "Endpoint IA"`}
        ...
        <div className="mt-3 text-xs text-yellow-700 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-950/40 border border-yellow-200 dark:border-yellow-900 rounded p-2.5">
          ⚠️ Sécurité : exposer Ollama sans auth est risqué. Si tu mets ce tunnel en permanence,
          ajoute un reverse proxy (Caddy/Nginx) avec Basic Auth ou un Bearer token.
        </div>
      </Card>
      ...
      <Card className="p-5">
        ...
        <p className="text-sm text-muted-foreground mb-3">
          Voici un <code className="text-xs bg-muted px-1 rounded">docker-compose.yml</code> minimal
          qui expose 4 serveurs MCP via un même tunnel Cloudflare.
        </p>
        <CodeBlock
  ...
  handle_path /fetch/*    { reverse_proxy mcp-fetch:8004 }
      ...
      <Card className="p-5">
        ...
        <p className="text-sm text-muted-foreground">
          Dans l'onglet <strong>Serveurs MCP</strong>, clique sur "Ajouter un serveur" et utilise
          les <strong>presets</strong>. Remplace{" "}
          <code className="text-xs bg-muted px-1 rounded">YOUR-TUNNEL</code> par ton URL (ex.{" "}
          <code className="text-xs bg-muted px-1 rounded">https://abc.trycloudflare.com</code>). Le
          bouton "Tester" vérifie la connexion et liste les outils exposés par chaque serveur.
        </p>
      </Card>
      ...
      <Card className="p-5">
        ...
        <ul className="text-sm text-muted-foreground space-y-1.5 list-disc pl-5">
          <li>
            Chaque appel passe par le cloud → ton tunnel → ta machine. Compte 200-500 ms de latence
            supplémentaire.
          </li>
          <li>
            Si ton PC s'éteint ou ton tunnel coupe, le chat ne marche plus — c'est inhérent au
            self-hosted via tunnel.
          </li>
          <li>
            Cette V1 n'a pas d'authentification. Reste sur un usage personnel ou ajoute du Basic
            Auth via Caddy/Cloudflare Access.
          </li>
          <li>
            Tous les modèles ne supportent pas les outils. Préfère{" "}
            <code className="bg-muted px-1 rounded">llama3.1</code>,{" "}
            <code className="bg-muted px-1 rounded">qwen2.5</code>,{" "}
            <code className="bg-muted px-1 rounded">mistral</code> avec mode
            instruct/function-calling.
          </li>
        </ul>
      </Card>
    </div>

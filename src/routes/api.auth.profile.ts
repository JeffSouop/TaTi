      ...
      PATCH: async ({ request }) => {
        ...
        if (newPassword && (!currentPassword || newPassword.length < 4)) {
          return Response.json(
            {
              ok: false,
              error: "currentPassword requis et newPassword doit avoir au moins 4 caracteres",
            },
            { status: 400 },
          );
        }
        ...
        if (nextEmail) {
          await pool.query(`UPDATE public.users SET email = $2 WHERE id = $1`, [
            me.id,
            nextEmail,
          ] as never);
        }
        if (nextFirstName !== null) {
          await pool.query(`UPDATE public.users SET first_name = $2 WHERE id = $1`, [
            me.id,
            nextFirstName,
          ] as never);
        }
        if (nextLastName !== null) {
          await pool.query(`UPDATE public.users SET last_name = $2 WHERE id = $1`, [
            me.id,
            nextLastName,
          ] as never);
        }
        if (nextAvatarUrl !== undefined) {
          await pool.query(`UPDATE public.users SET avatar_url = $2 WHERE id = $1`, [
            me.id,
            nextAvatarUrl,
          ] as never);
        }

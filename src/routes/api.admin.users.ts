...
export const Route = createFileRoute("/api/admin/users")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        if (!isAuthRequired())
          return Response.json({ ok: false, error: "Auth is disabled" }, { status: 400 });
      ...
      PATCH: async ({ request }) => {
        ...
        if (id === me.id && body.role) {
          return Response.json(
            { ok: false, error: "Un admin ne peut pas modifier son propre role" },
            { status: 400 },
          );
        }
        if (id === me.id && body.is_active === false) {
          return Response.json(
            { ok: false, error: "Un admin ne peut pas desactiver son propre compte" },
            { status: 400 },
          );
        }

        if (body.role) {
          await pool.query(`UPDATE public.users SET role = $2 WHERE id = $1`, [
            id,
            body.role,
          ] as never);
        }
        if (typeof body.is_active === "boolean") {
          await pool.query(`UPDATE public.users SET is_active = $2 WHERE id = $1`, [
            id,
            body.is_active,
          ] as never);
        }
      ...
      DELETE: async ({ request }) => {
        ...
        if (id === me.id)
          return Response.json(
            { ok: false, error: "Cannot delete current admin" },
            { status: 400 },
          );

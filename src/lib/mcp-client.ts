...
async function rpc<T = unknown>(
  ...
  const rpcPayload = payload as { error?: { message?: string }; result?: unknown };
  if (rpcPayload.error) {
    throw new Error(
      `MCP ${method} error: ${rpcPayload.error.message ?? JSON.stringify(rpcPayload.error)}`,
    );
  }
  return { result: rpcPayload.result as T, sessionId: newSessionId ?? undefined };
}
